...
def install_package():
    ...
    if system_version_id < 80:
        local.path(
            "/etc/libvirt/qemu.conf").write("group = 'hugetlbfs'", mode="a+")
...
def config_isolated_cpu_and_Gb_hugepage(str_cpus, num_hpage):
    ...
    local.path(
        "/etc/tuned/cpu-partitioning-variables.conf").write("isolated_cores={}".format(str_cpus))
...
@set_check(0)
def install_ovs():
    ovs_version = get_env("OVS_URL")
    pack_name = os.path.basename(ovs_version)[:-4]
    container_selinux_url = get_env("CONTAINER_SELINUX_URL")
    ovs_selinux_url = get_env("OVS_SELINUX_URL")
    ovs_url = get_env("OVS_URL")
    run(f"yum -y install {container_selinux_url} {ovs_selinux_url} {ovs_url}", "0,1")


@set_check(0)
def install_driverctl():
    driverctl_url = get_env("DRIVERCTL_URL")
    run(
        f"rpm -qa | grep `basename -s '.rpm' {driverctl_url}` || yum install -y {driverctl_url}")
...
def install_dpdk():
    ...
    run(
        f"rpm -qa | grep `basename -s '.rpm' {dpdk_url}` || rpm -ivh {dpdk_url}")
    run(
        f"rpm -qa | grep `basename -s '.rpm' {dpdk_tool_url}` || rpm -ivh {dpdk_tool_url}")
...
def init_netscout_config():
    dir_name = f"""{case_path}/NetScout"""
    with pushd(dir_name):
        netscout_host = get_env("NETSCOUT_HOST")
        if netscout_host != "":
            import base64
...
def init_physical_topo_without_switch():
    ...
    server_port_one = get_env("SERVER_PORT_ONE")
    client_port_one = get_env("CLIENT_PORT_ONE")
    server_port_two = get_env("SERVER_PORT_TWO")
    client_port_two = get_env("CLIENT_PORT_TWO")
    if get_env("CONN_TYPE") == "netscout":
...
def init_physical_topo_with_switch():
    init_netscout_tool()
    init_netscout_config()
    conn_type = get_env("CONN_TYPE")
    switch_port_one = get_env("SWITCH_PORT_ONE")
    switch_port_two = get_env("SWITCH_PORT_TWO")
    switch_port_three = get_env("SWITCH_PORT_THREE")
    switch_port_four = get_env("SWITCH_PORT_FOUR")
    server_port_one = get_env("SERVER_PORT_ONE")
    server_port_two = get_env("SERVER_PORT_TWO")
    client_port_one = get_env("CLIENT_PORT_ONE")
    client_port_two = get_env("CLIENT_PORT_TWO")
    ...
    if conn_type == "netscout":
        if os.environ["TRAFFIC_TYPE"] == "xena":
            traffic_port_one = get_env("TRAFFIC_PORT_ONE")
            traffic_port_two = get_env("TRAFFIC_PORT_TWO")
...
def config_hugepage():
    ...
    if i_am_server():
        server_nic1_mac = get_env("SERVER_NIC1_MAC")
        ...
        client_nic1_mac = get_env("CLIENT_NIC1_MAC")
...
def add_bridge_interface_to_xml(br_name, mac_addr, bus_info):
    ...
    with pushd(case_path):
        if os.path.exists("g1.xml"):
            result = bash(
                "cat g1.xml | virt-xml --remove-device --network type=vhostuser").value()
            if result:
...
def ovs_linux_bond_functional_test(bond_mode):
    ...
    if i_am_server():
        with enter_phase("create vf and get vf name"):
            ...
            server_nic1_mac = get_env("SERVER_NIC1_MAC")
            server_nic2_mac = get_env("SERVER_NIC2_MAC")
        ...
        with enter_phase("ping vm check the connection is ok"):
            ...
            connection_ping_check(
                gname, "ping {} -c 100".format(os.environ["CLIENT_GUEST_IP"]))
        ...
        with enter_phase("ovs linux bond failover test"):
            ...
            send_command(
                "sync_set CLIENT OVS_LINUX_BOND_FAILOVER_TIMEOUT_TEST_END")
            pass

    elif i_am_client():
        with enter_phase("create vf and get vf name"):
            client_nic1_mac = get_env("CLIENT_NIC1_MAC")
            client_nic2_mac = get_env("CLIENT_NIC2_MAC")
        ...
        with enter_phase("ovs linux bond failover test"):
            send_command(
                "sync_wait SERVER OVS_LINUX_BOND_FAILOVER_TIMEOUT_TEST_END")
            pass
    else:
        log("Here wrong rule")
...
def ovs_bond_functional_test(bond_mode):
    ...
    if i_am_server():
        ...
        with enter_phase(f"{bond_mode} failover test "):
            gname = xml_tool.xml_get_name(guest_xml)
            ovs_bond_failover_test(gname, get_env("CLIENT_GUEST_IP"))
...
def ovs_dpdk_bond_functional_test(bond_mode):
    ...
    if i_am_server():
        ...
        with enter_phase(f"{name} {bond_mode} config and start vm and config ip "):
            ...
            start_guest(guest_xml)
            g_name = xml_tool.xml_get_name(case_path + "/" + guest_xml)
            configure_guest(g_name, get_env("SERVER_GUEST_IP"))
        with enter_phase("ovs dpdk bond ping check"):
            cmd = """ ping {} -c 100 """.format(
                get_env("CLIENT_GUEST_IP"))
            gname = xml_tool.xml_get_name(guest_xml)
        ...
        with enter_phase(f"{name} {bond_mode} client side config vm xml and start vm "):
            ...
            start_guest(guest_xml)
            g_name = xml_tool.xml_get_name(case_path + "/" + guest_xml)
            configure_guest(g_name, get_env("CLIENT_GUEST_IP"))
...
def update_xml_sriov_vf_port(xml_file,vlan_id=0):
    ...
    vs2_bus_info = my_tool.get_bus_from_name(get_env("NIC2_VF"))
    vf1_bus_info.replace(":",'_')
    vs2_bus_info.replace(":",'_')
...
def ovs_dpdk_pvp_test(q_num,mtu_val,pkt_size,cont_time):
    ...
    if q_num == 1:
        xml_tool.update_image_source(newcase_path + "/" + get_env("one_queue_image"))
    else:
